pageextension 54501 "ExtendPostedSalesInvoice" extends "Posted Sales Invoice"
{
    ...
    actions
    {
        addafter("&Invoice")
        {
            ...
            action("LOLI_Download Excel")
            {
                ...
                trigger OnAction()
                begin
                    DownloadSalesInvExcelEmail(Rec);
                end;
            }
        }
    }
    ...
    var
        ...
        EmailOutStream_G: OutStream;
        EmailInStream_G: InStream;
    ...
    procedure DownloadSalesInvExcelEmail(recPostedSalesInv: Record "Sales Invoice Header")
    var
        FilePath_L: Text;
    begin
        Clear(FilePath_L);
        FilePath_L := Rec."No." + '.xlsx';
        IF Cust.GET(recPostedSalesInv."Sell-to Customer No.") then begin

            TempBlob.CreateInStream(EmailInStream_G, TextEncoding::UTF8);
            TempBlob.CREATEOUTSTREAM(EmailOutStream_G, TextEncoding::UTF8);

            Rec.SetRecFilter;
            RecRef.open(Database::"Sales Invoice Header");
            RecRef.SetView(Rec.GetView);
            RecRef.SetTable(SalesInvHeader);
            IF REPORT.SaveAs(54500, XMLParameters, ReportFormat::Excel, EmailOutStream_G, RecRef) THEN
                File.DownloadFromStream(EmailInStream_G, 'Save As Excel', '', '', FilePath_L);
        end;
        RecRef.Close();
    end;
}